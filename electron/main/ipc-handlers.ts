...
`;
    ...
    proc.on("close", (code) => {
      if (code === 0) resolve();
        ...
        reject(
          new Error(
            `setup.py exited with code ${code}\n${stderr.slice(-2000)}`,
          ),
        );
    });
...
export function setupIpcHandlers(
  ...
  const activeDownloads = new Map<
    string,
    { percent: number; file?: string; fileIndex?: number; totalFiles?: number }
  >();
  ...
  ipcMain.handle("log:getPath", () =>
    join(app.getPath("userData"), "logs", "modly.log"),
  );
  ipcMain.handle("log:readAll", async (_event, session?: string) => {
    ...
    for (const file of files) {
      try {
        ...
        result[file] = existsSync(filePath)
  ...
  ipcMain.handle(
    "setup:saveDataDir",
    async (_event, { baseDir }: { baseDir: string }) => {
      const userData = app.getPath("userData");
  ...
  ipcMain.handle(
    ...
    async (
      ...
      try {
        await axios.post(
          `${API_BASE_URL}/model/unload/${encodeURIComponent(modelId)}`,
          {},
          { timeout: 10_000 },
        );
  ...
  ipcMain.handle(
    "model:isDownloaded",
    (_, modelId: string, downloadCheck?: string): boolean => {
      const modelsDir = getSettings(app.getPath("userData")).modelsDir;
  ...
  ipcMain.handle("model:activeDownloads", () =>
    [...activeDownloads.entries()].map(([modelId, progress]) => ({
      modelId,
      ...progress,
    })),
  );
  ...
  ipcMain.handle(
    "model:export",
    async (_, { outputUrl, format }: { outputUrl: string; format: string }) => {
      const win = getWindow();
  ...
  ipcMain.handle(
    ...
    async (
      ...
      if (patch.hfToken !== undefined) {
        ...
        try {
          await axios.post(
            `${API_BASE_URL}/settings/hf-token`,
            { token: patch.hfToken },
            { timeout: 3000 },
          );
        } catch {
          /* FastAPI may not be running yet — ignore */
        }
      }
      return updated;
    },
  ...
  ipcMain.handle(
    "workspace:renameCollection",
    async (_, { oldName, newName }: { oldName: string; newName: string }) => {
  ...
  ipcMain.handle(
    ...
    async (
      _,
      {
        ...
        filename,
        meta,
      }: { collection: string; filename: string; meta: unknown },
    ) => {
      const metaFile = filename.replace(/\.glb$/, ".meta.json");
      await writeFile(
        workspacePath(collection, metaFile),
  ...
  ipcMain.handle(
    ...
    async (
      _,
      { collection, filename }: { collection: string; filename: string },
    ) => {
      await rmAsync(workspacePath(collection, filename), { force: true });
      await rmAsync(
        workspacePath(collection, filename.replace(/\.glb$/, ".meta.json")),
        { force: true },
      );
    },
  );
  ...
  ipcMain.handle(
    "fs:moveDirectory",
    async (_, { src, dest }: { src: string; dest: string }) => {
      try {
  ...
  async function fetchTrustedRepos(): Promise<Set<string>> {
    ...
    try {
      ...
      const repos = new Set(
        (data.trusted_repos ?? []).map((r: string) =>
    ...
    async (event, githubUrl: string) => {
      ...
      try {
        ...
        if (parsed.hostname !== "github.com")
          throw new Error(
            "Invalid URL: must be a GitHub repository (github.com)",
          );
        ...
        const response = await axios.get(tarballUrl, {
          ...
          onDownloadProgress: (evt) => {
            const pct = evt.total
              ? Math.round((evt.loaded / evt.total) * 80)
              : 40;
        ...
        if (!existsSync(manifestPath))
          throw new Error("manifest.json missing from repository");
        ...
        const {
          ...
          manifest,
          {
            hasEntryFile: (candidate) =>
              existsSync(join(extractDir, candidate)),
            hasGeneratorFile: () =>
              existsSync(join(extractDir, "generator.py")),
          },
          "repository",
        );
        if (!hasNodes)
          throw new Error(
            'manifest.json: required field "nodes" missing or empty',
          );
        ...
        const extensionsDir = getSettings(
          app.getPath("userData"),
        ).extensionsDir;
        await mkdir(extensionsDir, { recursive: true });
        const destDir = resolveExtensionPathWithinRoot(
          extensionsDir,
          extensionId,
        );
          ...
          if (isPythonProcess) {
            ...
            if (existsSync(join(destDir, "setup.py"))) {
              ...
              try {
                ...
                emit({
                  step: "setting_up",
                  message: `Warning: setup failed — ${err}`,
                });
              }
            }
          } else if (isProcess) {
            ...
            try {
              ...
              /* Python might not be running yet */
            }
          }
        ...
        if (tarPath && existsSync(tarPath))
          rmAsync(tarPath, { force: true }).catch(() => {});
        if (extractDir && existsSync(extractDir))
          rmAsync(extractDir, { recursive: true, force: true }).catch(() => {});
      }
    },
  );
  ...
  ipcMain.handle("extensions:uninstall", async (_, extensionId: string) => {
    ...
    const builtinPath = resolveExtensionPathWithinRoot(
    ...
    if (existsSync(builtinPath)) {
      return {
        success: false,
        error: `"${safeExtensionId}" is a built-in extension and cannot be uninstalled.`,
      };
    }

    const extensionsDir = getSettings(userData).extensionsDir;
    const extPath = resolveExtensionPathWithinRoot(
      extensionsDir,
      safeExtensionId,
    );
    try {
      ...
      try {
        ...
        /* ignore if Python is not running */
      }
      return { success: true };
    } catch (err) {
      return { success: false, error: String(err) };
  ...
  ipcMain.handle("extensions:repair", async (_, extensionId: string) => {
    try {
      const safeExtensionId = assertSafeExtensionId(extensionId);
      const extDir = resolveExtensionPathWithinRoot(
        getSettings(app.getPath("userData")).extensionsDir,
        safeExtensionId,
      );
      if (!existsSync(join(extDir, "setup.py"))) {
        return {
          success: false,
          error: "No setup.py found for this extension",
        };
      }
      const { sm: gpuSm, cudaVersion } = await detectGpuInfo();
      await runExtensionSetup(extDir, gpuSm, cudaVersion, (line) =>
        logger.info(`[ext-repair] ${line}`),
      );
      try {
        await axios.post(
          `${API_BASE_URL}/extensions/reload`,
        ...
        /* ignore if Python is not running yet */
      }
      return { success: true };
    } catch (err: any) {
  ...
  ipcMain.handle("extensions:reload", async () => {
    ...
    try {
      ...
      return {
        success: true,
        errors: (res.data as { errors?: Record<string, string> }).errors ?? {},
      };
    } catch (err) {
      return { success: false, error: String(err) };
    }
    ...
    async (
      ...
      try {
        ...
        if (manifest.type !== "process")
          return {
            success: false,
            error: `Extension "${extensionId}" is not a process extension`,
          };
        ...
        if (isPythonEntry) {
          const pythonExe =
            getExtPythonExe(extDir) ?? getVenvPythonExe(userData);
  ...
  ipcMain.handle(
    "workflows:save",
    async (_, workflow: { id: string; [key: string]: unknown }) => {
      try {
  ...
  ipcMain.handle("workflows:import", async () => {
    ...
    try {
      ...
      await writeFile(
        join(workflowsDir(), `${workflow.id}.json`),
