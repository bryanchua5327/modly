...
function IoBadge({ type }: { type: "image" | "text" | "mesh" }) {
  return (
    <span
      className={`inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium border ${IO_STYLES[type]}`}
    >
      {type}
    </span>
  );
    ...
    nodes: [
      {
        ...
        data: { enabled: true, params: {}, showInGenerate: true },
      },
      {
        id: outputNodeId,
        type: "outputNode",
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col items-start min-w-0">
        <span className="text-[11px] font-semibold text-zinc-400 group-hover:text-zinc-200 transition-colors truncate leading-tight">
          {title}
        </span>
        {author && (
          <span className="text-[9px] text-zinc-600 truncate leading-tight">
            {author}
          </span>
        )}
      </div>
...
function ExtensionsPanel({
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      setWidth((w) =>
        Math.min(PANEL_MAX, Math.max(PANEL_MIN, startW.current + delta)),
      );
    };
    const onUp = () => {
  ...
  const isExpanded = (id: string, hasMatches: boolean) =>
    (query && hasMatches) || !collapsed[id];

  // Base group
  const filteredBuiltinNodes = PANEL_BUILTIN_NODES.filter(
    (n) => !query || n.label.toLowerCase().includes(query),
  );
  const filteredBuiltinExts = allExtensions.filter(
    (e) => e.builtin && (!query || e.name.toLowerCase().includes(query)),
  );
  ...
  const nonBuiltinMap = useMemo(() => {
    const map = new Map<
      string,
      { extensionName: string; nodes: WorkflowExtension[] }
    >();
    for (const ext of allExtensions) {
      if (ext.builtin) continue;
      if (!map.has(ext.extensionId))
        map.set(ext.extensionId, {
      ...
      <div className="flex shrink-0" style={{ width }}>
        ...
        <div className="flex flex-col flex-1 min-w-0 bg-zinc-950/30">
          ...
          <div className="flex-1 overflow-y-auto px-3 py-2 flex flex-col gap-0.5">
              ...
              <div>
                ...
                {isExpanded("base", baseCount > 0) && (
                  <div className={`grid ${gridClass} gap-2 mt-1.5 mb-3`}>
                    {filteredBuiltinNodes.map(
                      ({ type, label, color, icon }) => (
                        <div
                          key={type}
                          draggable
                          onDragStart={(e) => {
                            e.dataTransfer.setData(DRAG_NODE_KEY, type);
            ...
            {[...nonBuiltinMap.entries()].map(
              ([extId, { extensionName, nodes }]) => {
...
const BUILTIN_NODES = [
  ...
  },
  {
    ...
    label: "Load 3D Mesh",
    color: "#a78bfa",
    description: "Load a 3D mesh file or use current model",
  },
  {
    type: "outputNode",
    label: "Add to Scene",
    color: "#a78bfa",
    description: "Output node — adds the mesh to the 3D scene",
  },
  {
    type: "previewNode",
    label: "Preview Views",
    color: "#38bdf8",
    description: "Displays multi-view image outputs in a 2×3 grid",
  },
  {
    type: "waitNode",
    label: "Wait",
...
function NodePalette({
  ...
  const nonBuiltinMap = useMemo(() => {
    ...
    for (const ext of allExtensions) {
      ...
      if (!map.has(ext.extensionId))
        map.set(ext.extensionId, {
          extensionName: ext.extensionName,
          extensionAuthor: ext.extensionAuthor,
          nodes: [],
        });
      map.get(ext.extensionId)!.nodes.push(ext);
    }
  ...
  const { groups, totalItems } = useMemo(() => {
    ...
    // Base group
    const filteredBuiltinNodes = BUILTIN_NODES.filter(
      (n) =>
        !q ||
    ...
    if (baseVisible) {
      ...
      if (baseExp) {
        filteredBuiltinNodes.forEach((n) =>
          items.push({ kind: "node", data: n, flatIdx: flatIdx++ }),
        );
        filteredBuiltinExts.forEach((e) =>
          items.push({ kind: "ext", data: e, flatIdx: flatIdx++ }),
        );
      }
    ...
    for (const [
      extId,
      { extensionName, extensionAuthor, nodes },
    ] of nonBuiltinMap) {
      const filtered = nodes.filter(
        (e) =>
          !q ||
      ...
      const items: PaletteGroup["items"] = [];
      if (expanded)
        displayNodes.forEach((e) =>
          items.push({ kind: "ext", data: e, flatIdx: flatIdx++ }),
        );
      groups.push({
        id: extId,
        title: extensionName,
        author: extensionAuthor || undefined,
        expanded,
        items,
      });
    }
...
function HelpModal({ onClose }: { onClose: () => void }) {
  ...
  useEffect(() => {
    window.electron.fs
      .readScreenshotDataUrl("workflow-helper.png")
      .then(setHelperImg)
      .catch(() => {});
  }, []);
  return createPortal(
        ...
        <div className="px-5 py-5 flex flex-col gap-5 overflow-y-auto">
          ...
          <section className="flex flex-col gap-2">
            ...
            <p className="text-[12px] text-zinc-300 leading-relaxed">
              ...
              . Each node receives data from its inputs (left handle) and
              produces a result on its output (right handle). Data flows from
              left to right — you connect nodes by dragging from one handle to
              another.
            </p>
          </section>
          ...
          <section className="flex flex-col gap-2.5">
            ...
            <div className="flex flex-col gap-2">
              <div className="flex items-start gap-3 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/40">
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium border border-sky-500/30 bg-sky-500/10 text-sky-400 shrink-0 mt-0.5">
                  image
                </span>
                <div>
                  <p className="text-[11px] font-medium text-zinc-200">Image</p>
                  <p className="text-[11px] text-zinc-500 mt-0.5 leading-relaxed">
                    Source node. Pick a local image file — it becomes the input
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/40">
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium border border-amber-500/30 bg-amber-500/10 text-amber-400 shrink-0 mt-0.5">
                  text
                </span>
                <div>
                  <p className="text-[11px] font-medium text-zinc-200">Text</p>
                  <p className="text-[11px] text-zinc-500 mt-0.5 leading-relaxed">
                    Source node. Pass a text prompt to extensions that accept
                    text input.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/40">
                <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium border border-violet-500/30 bg-violet-500/10 text-violet-400 shrink-0 mt-0.5">
                  mesh
                </span>
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500 mt-0.5 leading-relaxed">
                    Source node. Load a .glb, .obj, .stl or .ply file from disk,
                    or use the model currently loaded in the 3D viewer.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/40">
                <div className="flex gap-1 shrink-0 mt-0.5">
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium border border-sky-500/30 bg-sky-500/10 text-sky-400">
                    image
                  </span>
                  <span className="text-zinc-600 text-[9px] flex items-center">
                    →
                  </span>
                  <span className="inline-flex items-center px-1.5 py-0.5 rounded text-[9px] font-medium border border-violet-500/30 bg-violet-500/10 text-violet-400">
                    mesh
                  </span>
                </div>
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500 mt-0.5 leading-relaxed">
                    Runs a locally installed AI model to convert an image into a
                    3D mesh. Requires the model weights to be downloaded first
                    from the{" "}
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/40">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500 mt-0.5 leading-relaxed">
                    Transforms a mesh — examples: Optimize Mesh (polygon
                    reduction), Export Mesh (save to file). No GPU required.
                  </p>
                </div>
              </div>
              ...
              <div className="flex items-start gap-3 p-3 rounded-xl bg-zinc-800/50 border border-zinc-700/40">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-zinc-500 mt-0.5 leading-relaxed">
                    Terminal node. Receives the final mesh and loads it directly
                    into the 3D viewer when the workflow completes.
                  </p>
                </div>
              </div>
            </div>
          ...
          <section className="flex flex-col gap-2">
            ...
            <ul className="flex flex-col gap-1.5">
              {[
                ...
                [
                  "Drag handle → canvas",
                  "Auto-opens the palette to connect a new node",
                ],
                [
                  "Right-click a link",
                ...
                [
                  "Run",
                  "Saves & executes the workflow, result goes to the 3D scene",
                ],
              ].map(([key, desc]) => (
                <li
                  key={key}
                ...
                >
                  <span className="px-1.5 py-px rounded bg-zinc-800 border border-zinc-700 text-zinc-300 font-medium text-[10px] shrink-0 mt-px">
                    {key}
                  </span>
                  <span>{desc}</span>
                </li>
...
function getNodeOutputType(
  ...
  return allExts.find((e) => e.id === (node.data as WFNodeData)?.extensionId)
    ?.output;
}
...
function getNodeInputType(
  ...
  const ext = allExts.find(
    (e) => e.id === (node.data as WFNodeData)?.extensionId,
  );
...
function WorkflowCanvasInner({
  ...
  const [pendingDropPos, setPendingDropPos] = useState<{
    x: number;
    y: number;
  } | null>(null);

  const saveTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  const preflightToastTimer = useRef<ReturnType<typeof setTimeout> | null>(
    null,
  );
  ...
  useEffect(() => {
    ...
    return () => {
      if (preflightToastTimer.current)
        clearTimeout(preflightToastTimer.current);
    };
  ...
  const isValidConnection = useCallback(
    (connection: Connection) => {
      const srcType = getNodeOutputType(
        getNode(connection.source) as Node,
        allExtensions,
      );
      ...
      if (pending?.nodeId) {
        ...
        const edge = isSource
          ...
          : {
              ...
              targetHandle: pending.handleId ?? undefined,
            };
    ...
    <div className="flex flex-col flex-1 overflow-hidden">
      ...
      <div className="flex items-center gap-2 px-5 py-3 border-b border-zinc-800 shrink-0 bg-zinc-950/20">
        ...
        >
          ...
          >
            <path d="M3 7v6h6" />
            <path d="M3 13A9 9 0 1 0 5.7 6.3" />
          </svg>
        </button>
        ...
        >
          ...
          >
            <path d="M21 7v6h-6" />
            <path d="M21 13A9 9 0 1 1 18.3 6.3" />
          </svg>
        </button>
      ...
      <div className="flex-1 relative" onDragOver={onDragOver} onDrop={onDrop}>
        ...
        {!nodes.some(
          (n) =>
            n.type === "extensionNode" &&
            allExtensions.find(
              (e) =>
                e.id === (n.data as WFNodeData).extensionId &&
                e.type === "model",
            ),
        ) && (
          <div className="absolute top-3 left-1/2 -translate-x-1/2 z-10 pointer-events-none">
            <div className="flex items-center gap-1.5 px-2.5 py-1 rounded-md bg-accent/10 border border-accent/20 text-accent-light whitespace-nowrap">
              ...
              <span className="text-[10px] font-medium">
                No AI model node in this workflow — add one from the extensions
                panel to generate a 3D mesh.
              </span>
            </div>
          </div>
        )}
...
export default function WorkflowsPage(): JSX.Element {
  ...
  const { modelExtensions, processExtensions, loadExtensions } =
    useExtensionsStore();
  ...
  async function handleImport() {
    ...
    if (result.success && result.workflow)
      setActive((result.workflow as Workflow).id);
  }
          ...
          <div className="flex flex-1 flex-col items-center justify-center text-zinc-600 gap-3">
            ...
            <div className="flex items-center gap-2 mt-2">
              ...
              <button
                ...
                className="px-4 py-2 rounded-lg border border-zinc-700 text-zinc-300 text-xs font-semibold hover:bg-zinc-800 transition-colors"
              >
                Import
              </button>
            </div>
